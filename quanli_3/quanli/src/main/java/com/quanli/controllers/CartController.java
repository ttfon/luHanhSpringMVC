...
public class CartController {
   ...
   /**  @RequestMapping("/cart/2add/{phongid}")
    public String addToCart(@PathVariable(name="phongid")int phongid, HttpSession session)
    {
        HashMap<Integer,Cart> cartItems = (HashMap<Integer,Cart>) session.getAttribute("myCartItems");
        if(cartItems == null)
        {
       ...
       int sl = 0;
        if(cartItems.containsKey(phongid)==true)
                ...
                else
                {
                    ...
                    item.setSoluong(1);
                    cartItems.put(phongid, item);
                     Tour tour  =this.tourService.getTourById(phongid);
                    item.setTour(tour);
                    item.setSoluong(1);
                    cartItems.put(phongid, item);
                    sl=item.getSoluong();
