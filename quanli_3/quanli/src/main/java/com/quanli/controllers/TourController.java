...
public class TourController {
   ...
   private ThanhPhoService thanhPhoService;
    ...
    public String showtour(Model model,@RequestParam(name="name", required = false,defaultValue = "")String name)
    {
    ...
    public String showdetailtour(@PathVariable(name="id")int id,Model model,@RequestParam(name="name", required = false,defaultValue = "")String name)
    {
        Tour tour = this.tourService.getTourById(id);
        model.addAttribute("tour", tour);
        model.addAttribute("thanhpho", this.thanhPhoService.getAllTP());
        model.addAttribute("tour1", this.tourService.getTourByTP(name));
        return "detailtour";
    }
