            ...
            <h3>Danh sách phòng bạn đã đặt (<c:out value="${sessionScope.slItems}"/>)</h3>
             <table class="table">
                ...
                <c:forEach var="map" items="${sessionScope.myCartItems}">
                  ...
                  <c:if test="${sessionScope.giaTong!=0}">
                  ...
                  <button class="btn btn-danger" onclick="layidks()">Quay lại</button>
                  </c:if>
                   <c:if test="${sessionScope.giaTong==0}">
                       <button class="btn btn-danger" > <a href="<c:url value="/khachsan"/>" style="text-decoration: none" >Quay lại</a></button>
                   </c:if>
